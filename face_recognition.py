...
class Face_Recognition:
    def __init__(self,root):
        ...
        title_lbl.place(x=-200,y=0,width=1850,height=45)

        #First Image
        img_top= Image.open(r"college_images\course-203-biometric-person-identification-facial-recognition-concept-futuristic-low-polygonal-human-face_67515-466.jpg")
        img_top = img_top.resize((700,800))
        self.photoimg_top=ImageTk.PhotoImage(img_top)

        bg_img=Label(self.root,image=self.photoimg_top)
        bg_img.place(x=0,y=50,width=700,height=800)

        #Second Image
        img_bottom= Image.open(r"college_images\dormakaba-Blog-Post-pictures-_-1024-x-683-83.webp")
        img_bottom = img_bottom.resize((800,800))
        self.photoimg_bottom=ImageTk.PhotoImage(img_bottom)

        bg_img=Label(self.root,image=self.photoimg_bottom)
        bg_img.place(x=700,y=50,width=800,height=800)
        ...
        b1_1.place(x=340, y=650,width=200, height=40)
        ...
        def draw_boundary(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x, y, w, h) in features:
                ...
                if confidence>77:
                    ...
                    cv2.putText(img, "Unknown Face", (x, y - 5), cv2.FONT_HERSHEY_COMPLEX, 0.8, color, 3)
